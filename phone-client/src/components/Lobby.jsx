    ...
    return (
      <div style={styles.container}>
        ...
        <div style={styles.joinedCard}>
          ...
          <h2 style={{ color: '#27ae60', margin: '8px 0' }}>Joined!</h2>
          ...
          <div style={{ color: '#aaa', fontSize: '15px' }}>
            {players.map((p, i) => (
              <div key={p.id || i} style={{
                ...
                <span style={{ color: '#f1c40f' }}>{i + 1}.</span>
                <span>{p.name}</span>
                {i === 0 && <span style={{ marginLeft: 'auto', fontSize: '12px', color: '#f1c40f' }}>⭐</span>}
              </div>
            ))}
          </div>
        </div>
...
const styles = {
  ...
  title: {
    fontSize: '52px', fontWeight: 'bold', color: '#e94560',
    margin: 0, letterSpacing: '8px',
  },
  subtitle: { color: '#aaa', fontSize: '16px', marginTop: 4 },
  form: {
    display: 'flex', flexDirection: 'column', gap: 14,
    width: '100%', maxWidth: 320,
  },
  input: {
    padding: '14px 16px', fontSize: '18px', borderRadius: 12,
  ...
  joinBtn: {
    ...
    color: '#fff', cursor: 'pointer', marginTop: 8, transition: 'all 0.2s',
  },
  error: {
    marginTop: 16, padding: '10px 20px',
    background: '#c0392b33', border: '1px solid #e74c3c',
    borderRadius: 8, color: '#e74c3c', fontSize: '14px',
  },
  joinedCard: {
    textAlign: 'center', background: '#16213e', padding: '24px 32px',
    borderRadius: 16, marginBottom: 24, border: '2px solid #27ae60',
  },
  checkmark: {
    width: 50, height: 50, borderRadius: '50%', background: '#27ae60',
    color: '#fff', fontSize: '28px', display: 'flex', alignItems: 'center',
    justifyContent: 'center', margin: '0 auto 8px', fontWeight: 'bold',
  },
  waitingBox: { textAlign: 'center', padding: 20, width: '100%', maxWidth: 320 },
};
